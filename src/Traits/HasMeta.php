...
trait HasMeta {
	...
	public function scopeHasMeta( $query, $key ) {
		return $query->select( $query->getQuery()->from . '.*', 'meta.key', 'meta.value' )
		             ->leftJoin( 'meta', $query->getQuery()->from . '.id', '=', 'meta.meta_id' )
		             ->whereRaw( '`meta`.`meta_type` = \'' . addslashes( __CLASS__ ) . '\'' )
		             ->whereRaw( '`meta`.`key` = \'' . $key . '\'' );
	}
	
	public function scopeWhereMeta( $query, $key, $operator = '=', $value = null ) {
		...
		if ( is_array( $value ) ) {
			return $this->scopeHasMeta( $query, $key )->where( function( $query ) use ( $operator, $value ) {
				foreach ( $value as $i => $sub_value ) {
					...
					if ( ! $i ) {
						$query->whereRaw( '`meta`.`value` ' . $operator . ' \'' . $query_value . '\'' );
					} else {
						$query->orWhereRaw( '`meta`.`value` ' . $operator . ' \'' . $query_value . '\'' );
					}
		...
		return $this->scopeHasMeta( $query, $key )
		            ->whereRaw( '`meta`.`value` ' . $operator . ' \'' . $query_value . '\'' );
	}
